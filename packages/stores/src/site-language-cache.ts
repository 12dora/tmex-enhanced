...
// 键是浏览器级的裸 key（与 `vibeterm-ui` 同理），远端 node 的语言绝不写入。

import { type LocaleCode, SUPPORTED_LOCALES } from '@vibeterm/shared';
import { migrateLocalStorageKey } from './storage-migration';

export const SITE_LANGUAGE_CACHE_KEY = 'vibeterm.site.language';
const LEGACY_SITE_LANGUAGE_CACHE_KEY = 'tmex.site.language';
...
export function readCachedSiteLanguage(): LocaleCode | null {
  try {
    migrateLocalStorageKey(LEGACY_SITE_LANGUAGE_CACHE_KEY, SITE_LANGUAGE_CACHE_KEY);
