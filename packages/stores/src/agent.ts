  ...
  function setupClientHandlers(setState: SetState, getState: GetState): void {
    ...
    const handleErrorEvent = (sessionId: string, payload: AgentErrorEventPayload): void => {
      ...
      core.notifications.error(
        core.t('agent.toast.errorTitle', { title: session?.title ?? 'Agent' }),
        {
    ...
    const handleCredentialWarning = (
      ...
      core.notifications.warning(core.t('agent.toast.credentialWarningTitle'), {
        description: core.t('agent.toast.credentialWarningDescription', {
  ...
  return create<AgentState>()(
    persist(
      (set, get) => ({
        ...
        async loadSessions() {
          try {
            const sessionList = await fetchAgentSessions(core.apiClient);
            set(() => {
        ...
        async refreshSession(sessionId) {
          try {
            const refreshed = await fetchAgentSession(sessionId, core.apiClient);
            if (refreshed === null) {
        ...
        async createSession(deviceId, paneId, options) {
          try {
            ...
            core.notifications.error(error instanceof Error ? error.message : String(error));
            return null;
          }
        },
        ...
        async deleteSession(sessionId) {
          try {
            await deleteAgentSession(sessionId, core.apiClient);
        ...
        async loadHistory(sessionId) {
          ...
          try {
            ...
            const afterSeq = state.historyLoaded[sessionId]
              ...
              : -1;
            const messageList = await fetchAgentMessages(sessionId, afterSeq, core.apiClient);
            set((prev) => {
        ...
        async sendMessage(sessionId, text) {
          ...
          try {
            const payload = await sendAgentMessage(sessionId, text, core.apiClient);
        ...
        async enqueueMessage(sessionId, text, steer = false) {
          try {
            await enqueueAgentMessage(sessionId, text, steer, core.apiClient);
        ...
        async editQueuedMessage(_sessionId, itemId, text) {
          try {
            await editQueuedAgentMessage(itemId, text, core.apiClient);
          } catch (error) {
            core.notifications.error(error instanceof Error ? error.message : String(error));
          }
        },
        ...
        async withdrawQueuedMessage(_sessionId, itemId) {
          try {
            await withdrawQueuedAgentMessage(itemId, core.apiClient);
          } catch (error) {
            core.notifications.error(error instanceof Error ? error.message : String(error));
          }
        },
        ...
        async stopSession(sessionId) {
          try {
            const stopped = await stopAgentSession(sessionId, core.apiClient);
            if (stopped) {
        ...
        async decideConfirmation(sessionId, confirmationId, approved, reason) {
          ...
          try {
            const decided = await decideAgentConfirmation(
              confirmationId,
              approved,
              reason,
              core.apiClient
            ...
            if (decided === 'conflict') {
              ...
              removeLocally();
              try {
                const confirmations = await fetchAgentConfirmations(sessionId, core.apiClient);
                set((prev) => ({
                  pendingConfirmations: {
                    ...prev.pendingConfirmations,
