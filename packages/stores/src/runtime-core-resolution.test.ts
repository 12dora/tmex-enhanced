...
import type { BellPlayer, NotificationSink } from '@tmex/notifications';
import {
...
const { proxyDefaultNotificationSink, resolveRuntimeCore } = await import('./runtime');
...
describe('transport 解析优先级', () => {
  ...
  test('惰性回落不在解析时建 client', () => {
    ...
    const probe: GatewayConnection = {
      ...
      paneSinks: connection.paneSinks,
      selectMachine: connection.selectMachine,
      dispose: () => {},
    };
...
describe('服务面缺省与覆盖', () => {
  ...
  test('缺省 notifications 走可替换的默认代理', () => {
    expect(resolveRuntimeCore().notifications).toBe(proxyDefaultNotificationSink);
  });
