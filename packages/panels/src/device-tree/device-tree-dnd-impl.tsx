// dnd-kit 实体实现：这一坨（含 @dnd-kit/core + sortable + utilities，压缩后约 17 KB gz）
// 只在真的要拖时才用得上，由 `device-tree-dnd.tsx` 的门面按需 import，不进首屏 chunk。
// 门面之外，只有本身就在懒加载页里的 `device-folders` 直接静态引用 `useDeviceTreeSensors`。
