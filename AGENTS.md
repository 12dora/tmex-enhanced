...
- LLM思考过程应尽量用中文展示。
- **严禁触碰本机生产环境的 tmex**：即用户系统安装的常驻服务（launchd 守护，监听 9883）及其安装目录 `~/Library/Application Support/tmex/`（含 `resources/`、`data/tmex.db`、`app.env`、runtime 产物）。禁止写入/覆盖/删除其中任何文件、禁止 kill 或重启服务进程。验证代码改动一律在仓库内起临时实例（显式覆盖 `VIBETERM_FE_DIST_DIR`、`GATEWAY_PORT`、`VIBETERM_BIND_HOST` 等被 shell 继承的 app.env 变量）；生产更新只走正式发版 + `npx tmex-cli@version upgrade`（终端执行，`version` 替换为目标版本号或 `latest`），由用户自行执行。
