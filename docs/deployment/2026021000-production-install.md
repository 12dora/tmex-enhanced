...
生产安装走 GitHub Releases 的 `vibeterm-cli` 包（launchd / systemd 用户服务 + SQLite），不要再用仓库里的 Docker Compose 或手写 JWT。多机 mesh、登录、passkey / TOTP、直连与排障见 [hub / node 运维指南](../hub/2026082800-hub-node-operations.md)。
...
开发调试用仓库内 `bun run dev`，走 `development.env`，端口与生产 9883 错开。三套环境见 [三套环境](../env/2026061301-three-tier-env.md)。**不要**用生产库或安装目录做开发。
...
unit 使用 `KillMode=process`，stop / restart / crash 只杀 VibeTerm 主进程，不连带 tmux。跨 logout 存活需自行 `loginctl enable-linger <user>`，安装程序不会代开。详见 [进程存活](./2026061400-process-survival.md)。
