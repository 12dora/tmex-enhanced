...
1. 中继起在 13443：网页「接入中继」与 `vibeterm relay enroll https://relay.example.com`（不写端口）都能探到并接入；
2. 显式写 `https://relay.example.com:13443` 时只发一次请求，不做候选扫描；
3. Hub 同上，`vibeterm hub join https://hub.example.com --token …` 能探到端口；
4. `vibeterm init --role hub,node` 交互能选到建议端口，写出的 `VIBETERM_HUB_PUBLIC_URL` 带端口；
