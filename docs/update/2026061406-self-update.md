...
未安装场景回落 `<os tmpdir>/tmex-release-cache`，`TMEX_RELEASE_CACHE_DIR` 可覆盖）。目录里只有三种文件：

- `tmex-cli-<version>.tgz`——校验通过的整包。资产与平台无关，所以缓存键只有版本号。
- `tmex-cli-<version>.tgz.sha256`——sidecar，写在整包 rename 之后；**没有 sidecar 的整包一律当崩溃残留**。
- `tmex-cli-<version>.tgz.part`——下载中的半成品。

同一版本的并发下载由 `downloadVerifiedRelease` 的单飞表合并成一次（`isReleaseDownloadInFlight()` 对外
暴露「该版本正在下载」）。已校验的整包按 `size + mtime` 在进程内记忆，同一批 N 个节点复用同一个包时
只算一次 sha256；size 或 mtime 一变即重算，进程重启后从头校验。
...
| 文件 | 处置 |
| --- | --- |
| `tmex-cli-<v>.tgz` | `v` 不在 `keepVersions` → 删；在但缺 sidecar 且该版本不在下载中 → 删 |
| `tmex-cli-<v>.tgz.sha256` | `v` 不在 `keepVersions`，或对应 `.tgz` 已不在 → 删 |
| `tmex-cli-<v>.tgz.part` | 该版本正在下载 → 保留；否则 mtime 超过 `partTtlMs`（缺省 24 h）→ 删 |
| 其他任何文件/目录 | 删 |

调用点：

- **网关启动**（`runtime.ts` 的 `sweepReleaseCacheOnStartup()`，紧挨 `sweepOrphanTransferTemps()`）：
  `keepVersions: []` + `partTtlMs: 0`，整目录清空——此刻不可能有下载在途。失败只吞掉，不挡启动。
