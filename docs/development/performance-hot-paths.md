...
- 节点网关 `pane-input-pacer.ts`（挂在 `DeviceSessionRuntime`，本地与 SSH 连接共用）：纯鼠标序列的输入按序列拆开，**一次只写一个**，下一个要等 tmux 回 `%end`（字节真正进 pty）之后再看该 pane 的输出：应用以 DEC 2026 包帧（Claude Code）时，**看到帧结束序列就写下一条**（只留 1 ms）；不包帧的应用退化为「输出静默 3 ms 且距回执 ≥ 8 ms」；没有输出则按自适应回退 `clamp(4×EWMA(回执→首输出), 40, 250 ms)`。待写队列按**写入节拍**折算的 120 ms 时间预算封顶（节拍 = 有积压时相邻两次鼠标写入的间隔 EWMA，即应用每行的真实消化周期；`clamp(round(120/节拍), 2, 64)`），超出丢最新的滚轮事件，按下/松开/移动一律不丢；预算取小是为了和原生终端一样「画面跟手、停手即停」（2.0.4 首发用的 300 ms 预算 + 固定 8 ms 间隔在快机器上反而显得粘滞，2.0.5 改掉）。键盘、粘贴等常规输入排在前面的鼠标条目之后，鼠标条目是顺序屏障，常规输入之间仍可流水线；上报关闭（DECRST 1000/1002/1003）时撤销队列里与窗口里尚未写入的鼠标条目。丢弃按 pane 每 5 s 记一行 `[tmux][input-lane]`。
...
**取舍**：滚动速度上限就是应用自己的渲染速率（它每渲染一次只滚一行），无法绕过；预算封顶意味着快速甩动时超出应用能力的那部分行被丢掉，换来画面跟手、停手即停。慢速探针（30 ms/帧）120 Hz 发 100 行：改前 35 行 / 丢 63；2.0.5 后 46 行 / 应用侧 0 丢弃 / 派发一结束即收敛（无拖尾）。快速探针（5 ms/帧）甩动：2.0.4 有 150–500 ms 拖尾，2.0.5 无拖尾且多滚 6–25%。真实 Claude Code 本机甩动：消息约 45 条、帧间隔无一超过 33 ms、派发结束即收敛。
