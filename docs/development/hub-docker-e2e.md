...
| `VIBETERM_E2E_HUB_HOST` | `ai.jiefakj.com` | 公网主机名 / SNI / Caddy site / extra_hosts / Playwright `--map-host` |
| `VIBETERM_E2E_HUB_IP` | `43.248.129.233` | 公网 IP；rsync/ssh 目标、extra_hosts、clock/UDP preflight、`.compose-bind.yml` 回退、`VIBETERM_E2E_TURN_EXTERNAL_IP` 默认 |
...
| F | 本机 Playwright（`--map-host/--map-ip` 默认 `ai.jiefakj.com` / `43.248.129.233`）。Let's Encrypt：**不** `ignoreHTTPSErrors`。private-ca：传 `--insecure-tls`（F 的 TLS 断言更弱）。打开 `/login`，密码登录，侧栏见 node-a/node-b，node-a 终端打 marker；账号安全页虚拟 authenticator 注册 passkey，登出后再用 passkey 登录 |
