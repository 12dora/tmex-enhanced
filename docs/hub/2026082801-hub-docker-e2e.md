...
本 harness 只新增 `scripts/hub-e2e/`，不改应用源码。容器内 tmux 使用独立 socket（`vibeterm-hub` / `vibeterm-node-a` / `vibeterm-node-b`），不会碰到宿主机名为 `tmex` 的 session（用户的生产会话），也不会写本机生产安装目录。
...
| 非 hub 机 `enroll` 登录失败 | 期望 login JSON 带 `sid`，网关实际走 `x-vibeterm-set-session`/Set-Cookie | `764dbba` |

驱动跑在 `driver` 容器（同一 `vibeterm-e2e` 镜像，挂仓库，`NODE_EXTRA_CA_CERTS=/ca/ca.crt`）。不改宿主机 `/etc/hosts`。curl 走 `--cacert`（curl 不认 `NODE_EXTRA_CA_CERTS`）。
...
默认远端是有域名 + Let's Encrypt 的 `ai.jiefakj.com`（`root`、`/root/vibeterm-e2e`）。同一套脚本也可指向无域名、私有 CA、非 root ssh 用户的第二台机器（见下方环境变量与示例）。
...
2026-08-28 实测结果：A–G 全部 PASS；当时 D 的路径停在 relay（跨 NAT RTC 未建立）。本轮起 D2 对 `transport=dc` 硬断言，并增加 L1–L8 覆盖可到达的 LAN DC。
...
不删除 `vibeterm-e2e:split` 镜像（下次可 `VIBETERM_E2E_SKIP_BUILD=1`）。不碰本机生产 VibeTerm、不碰名为 `tmex` 的 tmux session（用户的生产会话）。
