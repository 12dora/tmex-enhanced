...
- `ws`：OPEN 载荷 `{auth}`；DATA 为原样 Borsh 帧。目标侧验 `auth` 后把流包装为 **carrier** 挂到新建的 `GatewaySession`。
  ...
  - 复验节奏：常规会话挂上后第一帧必校验，之后按 `WS_SESSION_VERIFY_MS`（5 分钟，与本地浏览器 socket 的 `touchSocket` 同节奏）复验；分享凭证按 `SHARE_WS_VERIFY_MS`（1 分钟）。登出 / key log 撤销走 `MeshHttp.onSessionsRevoked` 即时拆掉已挂载的会话，不等复验窗口。
