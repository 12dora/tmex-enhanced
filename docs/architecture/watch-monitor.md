...
本文描述 Watch 规则模型、三种触发类型（正则 / 不变 / LLM）的语义、LLM 介入点与降级、采样调度与连接管理、`assist-regex`；面向改动 `apps/gateway/src/watch/` 与 `packages/panels/src/watch/` 的开发者。
...
- 单测：`watch/evaluator.test.ts`（match/unchanged/reset/ignore/once/repeat/cooldown 各分支、卡住 N 分钟计时）、`watch/service.test.ts`（注入 now/tick 驱动时序；llm 型周期判断用 mock LLM HTTP server；fail-open / 摘要降级 / 告警只发一次；连续错误自动停用）、`api/watch.test.ts`（REST 校验、assist-regex）。
- e2e：`apps/fe/tests/watch.spec.ts`（规则 CRUD、assist-regex mock 回填、真实触发链路采样命中 → WS → toast）、`mobile-agent-watch.spec.ts`。
