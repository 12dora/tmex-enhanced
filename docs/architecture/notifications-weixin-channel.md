...
本文描述微信通知渠道：iLink 协议层、半主动推送语义、数据模型、API 与扫码登录流程；面向改动 `apps/gateway/src/weixin/` 与通知渠道注册表的开发者。消息指令（`status` / `run` / `approve` 等）见 [消息指令](./messaging-commands.md)。

## 背景

VibeTerm 的出站通知渠道有通用 Webhook（HMAC 签名）、Telegram Bot 与「微信」——后者让用户在**真·个人微信号**（非企业微信群、非公众号）上收到 VibeTerm 告警（终端响铃、终端通知、agent 完成/待确认、设备断连、watch 触发等）。
...
> `context_token` 的失效 TTL 官方无文档，是本子系统**头号风险**，只能靠实测确定（见「测试」）。

## 设计

### 架构总览

通知分发是 **`NotificationChannel` 注册表**：
...
channels = [WebhookChannel, TelegramChannel, WeixinChannel, MeshForwardChannel]
```
...
- **没有微信专属开关**：推送由全局 `enable_notification_push` / `enable_bell_push` 加 channel 级启停控制（曾有的 `enable_weixin_*` 列已由 `0012_naive_lizard.sql` 移除）。
...
- 单元/集成（默认 `bun test`）：iLink api/client、渠道注册表、weixin 服务半主动/notify-once、DB helpers、API 路由。
- **context_token TTL 实测**（需人工）：
...
- 缺图像内容时后端 fail-loud（不回退 token，避免前端坏图）。
