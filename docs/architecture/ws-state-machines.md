# ws-borsh-v1 状态机（Gateway + FE）

本文是 `vibeterm-ws-borsh-v1` 两端状态机的规范说明：WS 连接 / 重连、设备连接、canonical 首屏 / 订阅 / resize / bell / feed 与缓存分层；面向改动 `packages/ws-client` 与 `apps/gateway/src/ws/` 的开发者。wire 格式见 [协议规范](./ws-borsh-v1-spec.md)。
