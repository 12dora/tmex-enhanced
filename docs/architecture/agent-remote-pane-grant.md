...
本文描述远端窗格上的 agent 会话调用目标节点 `/api/mesh-internal/tmux/*` 时的按窗格授权模型：签发、绑定、有效期、吊销与兼容；面向改动 `apps/gateway/src/agent/pane-grant/` 的开发者。Agent 本身见 [终端 Agent](./terminal-agent.md)。
...
- **旧 X → 新 Y**：被 403 拒。X 升级后首次用户操作即自愈，无需人工干预（见 [已知问题](../known-issues.md) KI-10）。
