...
本文描述站点主题（亮 / 暗）跨端同步的 WS kind `KIND_SITE_THEME_UPDATE`（0x0801）：C2S / S2C schema、服务端 last-writer-wins 与前端不回送的约定；面向改动主题同步的开发者。

## 背景

VibeTerm 支持多端同时连接同一 gateway（桌面 + 手机 + 平板）。用户在任一端切换主题时，所有端需在 <1s 内同步。早期方案靠一条 HTTP 路由写库 + 前端轮询，延迟高且无实时保证；该路由已下线，站点外观的上行与广播只剩本文这一条 WS 通道。
...
2. 服务端校验 → `Date.now()` 分配 serverTimestamp → 写 SiteSettings DB → 更新内存 currentTheme → 触发 tmux window-style 更新 + TUI 主题通知注入（[mode 2031](./tui-theme-notify.md)）→ 广播 S2C 给所有已连接 ws clients（含发送方）
...
4. `handleSiteThemeChange(theme)` 触发 window-style 更新
5. `broadcastThemeChange(theme)` 触发 mode 2031 通知注入
