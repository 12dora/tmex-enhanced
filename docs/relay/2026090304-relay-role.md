...
2. `GET …/kdf` → `deriveSeed` → 签 enroll proof → `POST /api/relay/enroll` `mode:'join'`。
3. 开密封包；用包内令牌分页下载密钥日志并解密；`verifyKeyLogChain`；**`head_seq` 处记录的 hash 必须等于包内 `head_hash`**，日志更短也拒绝。
4. 回放（passkey 走链内 replay verifier）→ `commitJoin` → 自签证书 → 在写锁下连续追加 `admit-node`（自己）与换代 `meta-key`（封装给全部已承认节点，含自己）。
5. 落 `mesh_relays`（回放的 `set-relays` 投影；加入的这一台中继地址必须在表里）、`mesh_secrets`（`K_log` epoch 0 + 新 `K_meta`）。
6. HTTP 追加两条记录到中继（`admit` sidecar）→ 按新 head 重封并 `POST …/pack` → 写 env（`TMEX_ROLES=node` 或保留 `relay,node`，`TMEX_HUB_URL=''`）→ 重启。
...
节点侧 `POST /api/mesh/relay/pack`（node-session；body 同中继 `/pack`，另可带 `urls?: string[]` 只转发子集）把 FE/CLI 用当时持有的根种子密封好的包转发到 `mesh_relays` 里能连上的每一台中继。

请求体（网页 enroll 对话框、reauth、根轮换、根签追加之后刷新密封包时用同一形状）：

```jsonc
{
  "sealed_pack": "<b64url nonce‖ct>",
  "kdf_params": { "salt": "<b64url 16B>", "memory_kib": 65536, "iterations": 3, "parallelism": 1 },
  "root_epoch": 0,
  "head_seq": 4,
  "urls": ["https://relay.example"]   // 可选；省略则转发全部已配置中继
}
```

密封材料来自 `GET /api/mesh/relay/join-material`（`logKey` + 当前 attach 的 `token` / `tenantId`）加上 `GET /api/auth/keylog/head`。节点**不持有**根种子，所以密封发生在浏览器或 CLI。`tmex relay enroll` / `reauth` 与 CLI 改密在持有根种子时直接打各中继 `/pack`。
...
user_key_log  type CHECK 追加 'set-relays'、'meta-key'、`rename-node`（重建表）
```
...
| `GET /join-material` | 仅 relay 模式 → `{ logKey, relays: [{url, tenantId, token}] }` |
...
`POST /api/local/leave` 增加可选 `targetRole: 'standalone' | 'relay'`（默认 standalone）。`relay,node → relay` 只清 mesh 成员、保留中继运营表与中继 env 键；去 standalone 则两边都清，并删掉 `TMEX_RELAY_*`。`node` / `hub,node` 不能直接 `targetRole: relay`（400 `invalid_target`）。`GET /api/local/status` 在本机含 relay 角色时多一块 `relay: { publicUrl, hasPassword, tenantCount, nodesOnline, currentNodes }`，不含令牌或口令哈希。
...
- **中继模式下对端版本已写入 `peer_cache.version`**：`relay.list` 解开状态块后落库，解不开时回落缓存。
  `KEYLOG_RECORD_COMPAT` 那道门禁仍读 `nodes` 表（只有 hub 侧会写），因此 §7.3 的两处豁免暂时保留；
  要把过旧的中继对端真正拦下，还需要 `inspectHubAuthRecordCompat` 改读 `peer_cache.version`。
