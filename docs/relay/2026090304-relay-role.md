...
本机已经是同一账户的成员时，`performRelayPasswordJoin` 不重建用户、不改 env、不重启，只做三件事：
...
2. **追平并应用缺的记录**（含那条 `rotate-root-keep` 与带新令牌的 `set-relays`），再跑一次
   `RelaySecrets.reconcile()`。只改 `mesh_relays` 是不够的：网关下次启动会按**投影**整表重写这张表，
   把刚装上的令牌又盖回去。
3. 投影里确实没有更新的 `set-relays`（中继上也没有）时，就地用账户根钥补签一条并追加到中继，
   本机投影从此与新令牌一致，reconcile 稳定。
