...
tmux 操作一律用独立 socket，永远不要碰名为 `tmex` 的 session（用户的生产会话，改名后仍是这个名字）。
...
收尾只 `tmux -L vibeterm-relay-dev kill-server`，绝不在默认 socket 上执行。

租户侧起两个临时 node 实例（各自独立端口、`VIBETERM_TMUX_SOCKET` 与 db），
走 `vibeterm relay enroll` → `GET /api/mesh/relay/join-material` → `vibeterm hub join --token r3.…` → admit → relay 流。
...
| 判定 | 触发条件 | 行为 |
|---|---|---|
| `reuse` | 租户未被踢、`token_epoch ≥ min_token_epoch`，且 `known_token_hash` 等于当前令牌 | **不换发**。响应 `token: null`、`token_unchanged: true`，节点沿用本机已存的令牌 |
...
| 成员完全够不着密钥日志（令牌已作废，kick 之后尤其如此） | 在该机器上用**账户密码**跑 `vibeterm relay join <url> --tenant <id> --password`。本机已是同一账户时走「只换令牌」（rekey）分支：开密封包取当前令牌覆盖 `mesh_relays`、刷新 `K_log`，不重建本机用户、不改 env、不重启。根公钥对不上仍报 `local_user_exists` |
