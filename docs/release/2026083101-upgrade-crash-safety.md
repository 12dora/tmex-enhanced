...
日期：2026-08-31　范围：`packages/app`（`vibeterm-cli` 的 `init` / `upgrade` / `upgrade --apply-current-package` / `upgrade --repair`）、`install.sh`、`apps/gateway/src/system/upgrade.ts`（Web 触发的后台升级）。
...
<installDir>/
  ...
  backups/<txnId>/{vibeterm.db,vibeterm.db-wal,vibeterm.db-shm}
...
| `backup` | 复制 `vibeterm.db{,-wal,-shm}` 到 `backups/<txn>` | 服务已停，`current` 仍指向旧版 | 同 stopping：验证旧服务健康后才清场；失败则保留 journal+backup，非零退出 |
