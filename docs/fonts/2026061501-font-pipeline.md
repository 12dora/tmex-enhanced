...
> 默认字体 Geist Mono 沿用仓库已有的扁平 woff2（`packages/theme/resources/fonts/GeistMonoNerdFontMono-*.woff2`，已在 `index.css` 静态 `@font-face`、终端首屏即用），工具不重新下载，仅在 manifest 中以默认项引用既有文件。
