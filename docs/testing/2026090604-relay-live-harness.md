...
从源码拉起若干 tmex runtime，走真实 CLI 与真实 HTTP 接口把它们并成一套 mesh，
...
7. 生成 `r3.` 加入码（见下节），在 B 上跑
   `tmex hub join --token r3.<…> --name relay-node-b --no-restart --install-dir <B>`。
...
- **`tmex enroll` 不支持中继**：它只走 hub 的 enrollment 路径。`r3.` 加入码目前只有网页
...
- **`Cookie` 头要带两个**：`/n/<B>` 的请求同时需要 A 的 `tmex_s_self` 与 B 的
  `tmex_s_<Bid>`（`apps/gateway/src/auth/cookies.ts`）。state 里 `a.cookie` 已经把两个都拼好，
  `b.viaA.cookie` 是同一串，可以直接 `curl -H "Cookie: …"`。
- **临时实例的 tmux session 名就叫 `tmex`**，但它在专用 socket（`tmux -L tmex-relay-e2e-r`）里，
...
```jsonc
{
  ...
  "relay": {
    ...
    "nodeId": "<R 的节点编号>", "tmuxSocket": "tmex-relay-e2e-r",
    "installDir": "…/relay", "cookie": "tmex_s_self=…"
  },
  "a": {
    "role": "node", "name": "tmex", "port": 19852, "peerPort": 39852,
    "baseUrl": "http://127.0.0.1:19852", "nodeId": "<A 的节点编号>",
    "tmuxSocket": "tmex-relay-e2e-a", "installDir": "…/a",
  ...
  "b": {
    ...
    "transport": "relay", "tmuxSocket": "tmex-relay-e2e-b", "installDir": "…/b",
    "cookie": "tmex_s_self=…",
    "viaA": { "url": "http://127.0.0.1:19852/n/<Bid>", "cookie": "tmex_s_self=…; tmex_s_<Bid>=…" }
  },
