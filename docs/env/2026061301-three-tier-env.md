...
VibeTerm 此前只有两个事实上的 `NODE_ENV` 取值（`development` / `production`），环境变量加载逻辑散落在 `dev-supervisor.sh`、两个测试 preload、`playwright.config.ts` 等多处并互相打补丁；dev / test 运行还会继承 shell 里安装版 `app.env` 的变量（`NODE_ENV=production`、生产 `DATABASE_URL`、指向安装目录的 `VIBETERM_MIGRATIONS_DIR`/`VIBETERM_FE_DIST_DIR`），多次导致 dev 启动崩溃、单元测试误写生产库。
...
读侧代码一律只读 `VIBETERM_*`，不要再判断前缀。全网升级到 ≥ 2.0 之后可以删掉整层别名，见 [改名迁移](../release/2026090607-rename-vibeterm.md)。
