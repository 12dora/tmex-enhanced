...
function clientObservedPanes(client: ServerWebSocket<ClientState>, deviceId: string): Set<string> {
  const panes = new Set<string>();
  const selected = client.data.borshState.selectedPanes[deviceId];
  if (selected) panes.add(selected);
  const subscribed = client.data.borshState.subscribedPanes[deviceId];
...
export class LegacyFeedBroadcaster {
  ...
  private readonly clientLegacyObservers = new WeakMap<
  ...
  syncLegacyPaneObservers(client: ServerWebSocket<ClientState>, deviceId: string): void {
  ...
  releaseLegacyPaneObservers(client: ServerWebSocket<ClientState>, deviceId?: string): void {
