...
import { JSON_BODY_MAX_BYTES, readJsonObjectBody } from './http';
...
describe('readJsonObjectBody', () => {
  ...
  test('returns null when content-length exceeds the cap', async () => {
    ...
    expect(await readJsonObjectBody(req)).toBeNull();
    expect(
      await readJsonObjectBody(jsonRequest('{"ok":true}', { 'content-length': '64' }), 16)
    ).toBeNull();
  });
  ...
  test('returns null when a chunked body exceeds the cap without content-length', async () => {
    ...
    expect(await readJsonObjectBody(req, 16)).toBeNull();
  });
