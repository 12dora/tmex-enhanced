...
CREATE TABLE `__new_user_key_log` (
	...
	CONSTRAINT "user_key_log_type_check" CHECK("type" in ('add-passkey', 'remove-passkey', 'rotate-root', 'set-totp', 'clear-totp', 'admit-node', 'revoke-node', 'reset-root', 'admit-hub', 'retire-hub', 'rotate-root-keep', 'set-relays', 'meta-key', 'rename-node', 'readmit-node', 'notification-sink'))
);
