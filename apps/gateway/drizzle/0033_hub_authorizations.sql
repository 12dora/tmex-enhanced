...
CREATE TABLE `__new_user_key_log` (
	...
	CONSTRAINT "user_key_log_type_check" CHECK("type" in ('add-passkey', 'remove-passkey', 'rotate-root', 'set-totp', 'clear-totp', 'admit-node', 'revoke-node', 'reset-root', 'admit-hub', 'retire-hub'))
);
...
CREATE TABLE `user_hub_authorizations` (
	`user_id` text NOT NULL,
	...
	PRIMARY KEY(`user_id`, `hub_node_id`),
	FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "user_hub_authorizations_status_check" CHECK("status" in ('active', 'retired'))
);
--> statement-breakpoint
CREATE UNIQUE INDEX `user_hub_authorizations_user_id_hub_node_id_unique` ON `user_hub_authorizations` (`user_id`,`hub_node_id`);
