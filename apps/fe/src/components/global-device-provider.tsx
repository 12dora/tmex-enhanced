...
function useDeviceIntentState(): DeviceIntentState {
  const [persistedConnectedDeviceIds, setPersistedConnectedDeviceIds] =
    usePersistedDeviceIds(CONNECTED_STORAGE_KEY);
  const [explicitlyDisconnectedDeviceIds, setExplicitlyDisconnectedDeviceIds] =
    usePersistedDeviceIds(DISCONNECTED_STORAGE_KEY);
...
function useRouteDeviceSubscription(
  ...
  useEffect(() => {
    const currentDeviceId = matchRouteDeviceId(location.pathname);
...
export function GlobalDeviceProvider({ children }: GlobalDeviceProviderProps) {
  ...
  const intent = useDeviceIntentState();
  ...
  useRouteDeviceSubscription(devicesData, ensureDeviceSubscribed);
