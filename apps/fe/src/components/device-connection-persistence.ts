import { defaultRuntime } from '@tmex/stores';
...
/**
 * 持久化的设备连接意图存储键。
 * 载荷格式固定为 `string[]`；键名带 runtime 前缀做命名空间隔离，
 * 若将来需要变更载荷结构，应改用新键名而非原地升级，避免旧版本读到无法解析的数据。
 */
export const CONNECTED_STORAGE_KEY = `${defaultRuntime.storagePrefix}tmex:connectedDevices`;
export const DISCONNECTED_STORAGE_KEY = `${defaultRuntime.storagePrefix}tmex:disconnectedDevices`;
