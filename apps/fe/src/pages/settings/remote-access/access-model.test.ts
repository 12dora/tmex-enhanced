...
describe('目标主机名与可应用性', () => {
  test('优先用 Access 应用已覆盖的主机名，其次用隧道配置的', () => {
    expect(accessTargetHostname(status())).toBe('tmex.example.com');
    expect(
      accessTargetHostname(status({ access: { ...status().access, hostname: 'old.example.com' } }))
    ).toBe('old.example.com');
    expect(
      accessTargetHostname(status({ config: { ...status().config, mode: 'off', hostname: null } }))
    ).toBeNull();
  });

  test('凭证、主机名、规则三者齐全才能应用', () => {
  ...
  test('同步比应用宽松：没有隧道主机名时也能匹配探测到的系统隧道主机名', () => {
