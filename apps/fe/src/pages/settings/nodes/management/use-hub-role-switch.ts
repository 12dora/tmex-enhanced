...
//   3. 目标升主，带 `newEpoch = max(已知 writerEpoch) + 1`。
//   4. 目标落库后**自己重启**，`/n/<目标>/api/...` 会断一段时间：轮询 `roleStatus` 时把不可达
//      当成「重启中」继续等，只有超出预算才判失败；随后再等入口的 `/api/mesh/hubs` 把
//      `writerHubId` 换成目标，这才算真的接管了写入。
//
// 切换跨越目标的一次重启，用户很可能在中途刷新页面：`{operationId, targetHubId, fromHubId}`
// 落 sessionStorage，挂载时据此接着轮询第 4 步。记录只在这一个标签页里，换标签页看不到——
// 它不是锁，只是「刷新即丢」的补丁。
...
/** 纪元只增不减：新主必须带一个比集合里所有已知值都大的纪元，否则会被 `HUB_EPOCH_STALE` 拒。 */
export function nextWriterEpoch(hubs: MeshHubEndpoint[]): number {
  let max = 0;
  for (const hub of hubs) if (hub.writerEpoch > max) max = hub.writerEpoch;
  return max + 1;
}
...
export function planHubRoleSwitch(input: HubRolePlanInput): HubRoleSwitchPlan | null {
  ...
  return {
    ...
    newEpoch: nextWriterEpoch(input.hubs),
  };
}
...
export function hubRoleSteps(t: Translate, plan: HubRoleSwitchPlan): string[] {
  ...
  if (plan.target) {
    steps.push(t('nodes.hubs.role.stepPromote', { target, epoch: plan.newEpoch }));
...
async function runDemoteOnly(p: HubRoleRunParams): Promise<HubRoleRunOutcome> {
  p.phase('demoting');
  const outcome = await p.io.role(p.plan.origin.nodeId, {
    mode: 'standby',
    operationId: p.operationId,
  });
  if (outcome.kind === 'ok') return { kind: 'done' };
  if (outcome.kind === 'unreachable') {
    return { kind: 'unconfirmed', message: p.t('nodes.hubs.role.errors.unreachable') };
  }
  return {
    kind: 'failed',
    message: p.t('nodes.hubs.role.failed', { error: hubRoleErrorText(p.t, outcome.code) }),
  };
}

export async function runHubRoleSwitch(p: HubRoleRunParams): Promise<HubRoleRunOutcome> {
  const target = p.plan.target;
  if (!target) return runDemoteOnly(p);
  ...
  if (p.plan.needsAdmit) {
    ...
    if (admit.kind === 'failed') {
      return {
        kind: 'failed',
        message: p.t('nodes.hubs.role.failed', { error: hubRoleErrorText(p.t, admit.code) }),
      };
    }
  ...
  // 原主不可达时跳过降备：升主带的更高纪元本身就把它围栏成 standby。
  if (p.plan.from && !p.plan.fromUnreachable && p.plan.from.nodeId !== target.nodeId) {
    p.phase('demoting');
    const demoted = await p.io.role(p.plan.from.nodeId, {
      mode: 'standby',
      operationId: p.operationId,
    });
    if (demoted.kind === 'failed') {
      return {
        kind: 'failed',
        message: p.t('nodes.hubs.role.failed', { error: hubRoleErrorText(p.t, demoted.code) }),
      };
    }
  }

  p.phase('promoting');
  const promoted = await p.io.role(target.nodeId, {
    mode: 'active',
    writerEpoch: p.plan.newEpoch,
    operationId: p.operationId,
  });
  if (promoted.kind === 'failed') {
    return {
      kind: 'failed',
      message: p.t('nodes.hubs.role.failed', { error: hubRoleErrorText(p.t, promoted.code) }),
    };
  }
  // `unreachable`：202 的回包可能丢在目标的重启里，目标却已经受理——只能靠回读确认。
  p.onAccepted?.();
  return awaitHubRoleSwitch({
...
export function useHubRoleSwitch(
  ...
  const finish = useCallback(
    (outcome: HubRoleRunOutcome, targetName: string) => {
      if (outcome.kind === 'done') toast.success(t('nodes.hubs.role.done', { target: targetName }));
  ...
  // 续跑只在挂载时跑一次，因此依赖一律走 ref：把 `finish` / `nameOf` 放进依赖数组，
  // 节点列表每刷新一次就会重跑这个 effect，cleanup 顺手把还在轮询的续跑掐掉。
  const resumeRef = useRef({ finish, io, nameOf, now, t });
  resumeRef.current = { finish, io, nameOf, now, t };

  // 刷新页面时切换多半还卡在目标的重启里：接上第 4 步，不让用户对着一份「已完成」的假象。
  useEffect(() => {
    const deps = resumeRef.current;
    const record = loadHubRoleSwitch(deps.now());
    if (!record) return;
    const controller = new AbortController();
    setRunning(true);
    setSwitchingIds(
      new Set(record.fromHubId ? [record.targetHubId, record.fromHubId] : [record.targetHubId])
    );
    void (async () => {
      ...
      resumeRef.current.finish(outcome, resumeRef.current.nameOf(record.targetHubId));
    })();
  ...
  const confirm = useCallback(() => {
    ...
    const targetName = plan.target?.name ?? plan.origin.name;
    ...
    setSwitchingIds(
      new Set([
        plan.origin.nodeId,
        ...(plan.target ? [plan.target.nodeId] : []),
    ...
    void (async () => {
      const outcome = await runHubRoleSwitch({
        ...
        onAccepted: () =>
          saveHubRoleSwitch({
            operationId,
            targetHubId: plan.target?.nodeId ?? plan.origin.nodeId,
            fromHubId: plan.from?.nodeId ?? null,
            startedAt: now(),
          }),
        t,
        admit: (target) =>
          admitHubSigned({ api, mode, prompt, io, target, setForce, forceResolve }),
      });
      ...
      finish(outcome, targetName);
    })();
  }, [api, finish, io, mode, now, plan, prompt, t]);
  ...
  return useMemo(
    ...
    [confirm, dismiss, force, phase, plan, request, resolveForce, running, stateOf, switchingIds]
  );
}
...
function randomOperationId(): string {
  try {
    ...
    // 非安全上下文没有 randomUUID：退到时间戳 + 随机数，够当幂等键。
  }
