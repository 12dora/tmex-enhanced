...
## G1 — Gateway mesh forwarder: pending-stream leak + skip decoding bulk pane data + frame-sizer cache bound
Read `prompt-archives/2026083003-perf-smell-gates-round6/sub/X3-report.md` items 1, 3 and 7 (authoritative; verify each claim in code before changing).
### Scope
apps/gateway/src/mesh/forwarder.ts (+ its tests), apps/gateway/src/mesh/mesh-http.ts (only if the close path needs it), apps/gateway/src/ws/canonical/frame-sizer.ts (+ tests), apps/gateway/src/ws/canonical-feed-session.ts (only the detach hook if needed), and if you need a bounded header-only reader for canonical events: a new small function next to the existing canonical codec in packages/shared/src (find where `PaneData` canonical events are encoded/decoded; add a `peek`-style reader that validates the header and skips the length-prefixed data without allocating it; keep it in the same file or a sibling file, with tests).
### Items
1. Leak: when the browser socket closes before the remote stream `open`, delete the `pendingStreams` entry and close the remote stream; add an identity-checked expiry as a second net. Test: open → browser close before open → map size back to 0, remote stream closed.
2. `StreamReplayState.noteInbound()`: for `PaneData` events extract device/pane/epoch/seq via the header-only reader and do NOT decode the data field; keep full decode for other event kinds. Malformed frames must be rejected exactly as before (test both). Measure before/after with a 10 MiB synthetic stream (report ms).
3. `frame-sizer` `maxDataByKey`: key by the UTF-8 byte lengths the result actually depends on (or evict on device detach) — pick the smaller change; add a test that cycling 10k unique ids does not grow the map unboundedly.
Verify: `cd apps/gateway && bun test src/mesh src/ws`, tsc ≤ 21.

Write your result to: /Users/konata/code/tmex-enhanced-wt-r6/prompt-archives/2026083003-perf-smell-gates-round6/sub/G1-result.md
