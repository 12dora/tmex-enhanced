...
## P2a — Terminal I/O path: single-pass LF normalization, batched history replay, time-bounded output coalescing
Read `prompt-archives/2026083003-perf-smell-gates-round6/sub/X2-report.md` findings "LF normalization", "history page replays", "coalescing only spans one task" (authoritative; verify in code).
### Scope
packages/terminal-ui/src/components/normalization.ts (+ tests), packages/terminal-ui/src/components/terminal-snapshot.ts (+ tests), packages/terminal-ui/src/components/TerminalSurface.ts (+ tests), packages/ws-client/src/pane-output-coalescer.ts (+ tests). Do NOT touch packages/ghostty-terminal or ws-client decoding (other agents).
### Items
1. Normalization: one pass, reuse a growable scratch buffer (return a subarray view or copy only the needed length), preserve `previousEndedWithCR` across chunks; byte-for-byte identical output to the old implementation (keep the old function in the test file as reference and fuzz 1,000 random chunk splits). Measure 10 MiB / 320 chunks before/after.
2. History paging: batch pages that arrive within a short burst (e.g. until the fetch loop yields / a ~16 ms window / the final page) and call `writeSnapshot()` once for the batch; keep immediate commit for the final page and for any live-output rebase ordering (read how live output and history interleave — that ordering must not change). Add a test with 22 fake pages counting `writeSnapshot` calls, and a benchmark with the real Ghostty parser if there is an existing harness (there is one in the terminal-ui tests) — report ms.
3. Coalescer: replace the microtask-only scheduler with a bounded scheduler: flush when ≥ threshold bytes, or on a ~4 ms timer / rAF where available, and immediately before reset/history/snapshot/resize events (read the callers). Document the max added latency in the result. Tests for: same-task merge, cross-task merge within the window, forced flush on control events.
Verify: `cd packages/terminal-ui && bun test`, `cd packages/ws-client && bun test`, tsc baselines.

Write your result to: /Users/konata/code/tmex-enhanced-wt-r6/prompt-archives/2026083003-perf-smell-gates-round6/sub/P2a-result.md
