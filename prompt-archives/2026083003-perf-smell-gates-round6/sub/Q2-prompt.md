...
## Q2 — panels: bounded tool-result preview in ToolCallCard
Read `prompt-archives/2026083003-perf-smell-gates-round6/sub/Y1-report.md` item 4 (verify in code first).
### Scope
packages/panels/src/agent/messages/tool-call-card.tsx and sibling files under packages/panels/src/agent/messages/ (+ tests); i18n keys only inside the `agent` sub-object of the three locale JSONs (you may run `bun run build:i18n`). Do NOT touch chat-thread.tsx, streaming-markdown, stores, apps/fe.
### Items
1. When the details dialog is open, render at most a bounded preview of the output (e.g. first 64 KiB or first 2,000 lines, whichever first) inside the `<pre>`, with a note "showing first N of M" and a "copy full output" action (navigator.clipboard with the retained full string). Copy remains on the full string.
2. Restrict image extraction (`asImageSrc` and the per-render scan) to outputs below a size threshold (e.g. 512 KiB) and to values that already look like image data URIs / known image-producing tools; and memoize it on the output value so it does not re-run each render.
3. Tests: SSR of an open card with 500 KiB output produces bounded HTML (< 100 KiB) and includes the note; small outputs unchanged; image detection skipped for big outputs.
Verify: `cd packages/panels && bun test && bunx tsc --noEmit -p .` (baseline 606/0, tsc 0), biome.

Write your result to: /Users/konata/code/tmex-enhanced-wt-r6/prompt-archives/2026083003-perf-smell-gates-round6/sub/Q2-result.md
