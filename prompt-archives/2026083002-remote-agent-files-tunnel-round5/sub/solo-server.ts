...
async function main() {
  const dir = `${ROOT}/solo`; mkInst(dir); const port = Number(process.env.SOLO_PORT ?? 21700), pp = await freePort(39700);
  ...
  startLoop(dir, 'solo'); await waitHealthy(port); log(`solo healthy on ${port}; dir=${dir}`);
  await Bun.write(`${ROOT}/solo.ready`, String(port));
  await new Promise(() => {});
}
