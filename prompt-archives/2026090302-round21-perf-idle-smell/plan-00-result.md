...
## 十、上线后实测（诚实记录：待机 CPU 未能证明改善）

1.1.21 已替换本机（`current -> versions/1.1.21`，healthz 200，install-meta `cliVersion: 1.1.21`）。

| 指标 | 本轮开始 | 上线后（重启 3 min 后、5 min 窗口） |
|---|---|---|
| 网关 CPU | 5.65%（一次采样）／3.16%（另一次采样） | **4.92%** |
| 日志增长 | 81 MB 累积、无轮转 | 4.98 MiB/天，**轮转已生效**（82 MB 滚成 `tmex.log.1`，活动文件重新从 0 开始） |

**结论要说清楚：待机 CPU 没有可证明的改善**，4.92% 落在基线两次采样（5.65% / 3.16%）之间，属噪声范围。

原因是本机**根本不处于待机**：`ws-metrics` 显示 `source_events≈265 / source_bytes≈21 KB` 每 30 秒，
即常年有约 9 事件/秒的终端输出（用户自己的 tmux pane 一直在跑东西）。本轮削减的是「**安静时**的开销」——
指标空转行、全量 pane 清扫、`os.networkInterfaces()`、key-log SELECT、会话续期写库、lag 采样。
这些在有持续输出时占比很小；真正吃 CPU 的是终端输出管线本身，而「按 pane 订阅、没人看的 pane 不走完整流水线」
（EX2 的 O18）本轮按风险明确不做。

同理，日志仍有约 5 MiB/天：全零抑制只在计数全为零时生效，而这台机器一直有输出，所以 `ws-metrics` 照常每 30 s 一行。
**真正兑现的是轮转**：从「无上限增长到 81 MB」变成「16 MiB × 3 代封顶」。

要在这台机器上看到待机收益，需要下一轮做 O18（按 pane 订阅）；或者在一台真正空闲的节点上对比。
PWA 侧的节电（光标闪烁 −180 唤醒/分钟、键盘循环 60 fps → 收敛后停、诊断轮询隐藏即停）是结构性的，
不依赖机器是否空闲，但需要在手机上用 Web Inspector 复测才能给出数字。
