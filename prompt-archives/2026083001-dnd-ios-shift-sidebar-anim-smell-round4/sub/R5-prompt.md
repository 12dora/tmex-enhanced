...
RESULT FILE: /Users/konata/code/tmex-enhanced/prompt-archives/2026083001-dnd-ios-shift-sidebar-anim-smell-round4/sub/R5-result.md

YOUR SCOPE (R5 — packages/app runtime assembly):
File: `packages/app/src/runtime/assemble.ts` (581 lines) + `assemble.test.ts`. Nothing else.
- `assembleTmex` (~153, CC17 / 365 lines) with inner `dispatch` (~285, CC26/60): TLS lifecycle, local/setup, Hub, mesh guards, gateway, frontend fallback and websocket dispatch are inlined. Extract the HTTP pipeline (an ordered array of `(req) => Response | null | Promise<...>` handlers is acceptable if it is shorter than the if-chain), the websocket router and the TLS lifecycle builder as top-level functions, preserving the current priority order exactly (tests `assemble.test.ts:117`, `:517`, `:647`, `:866`).
- Drop `export` from symbols with no importer outside the file (verify with `rg`).
Target: net -50 lines or better; `assembleTmex` under 150 lines, `dispatch` CC under 12.
