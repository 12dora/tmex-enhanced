...
## 2026-08-29 第三轮需求（测试反馈）

> 任务：
> 1. 密码错误登录失败文案提示所有节点均登录失败, 应该改为对应提示
> 2. Passkey注册入口应该在设置里, 而不是在登录窗
> 3. 登录页包含大量内部状态文案, 例如"一次密码或一次passkey…" 应该删除
> 4. 这些页面左上角top bar都应该展示tmex的logo和名称. 你应该使所有对logo和名称的引用归为一处,避免后续散弹修改
> 5. 登录成功时, 应该瞬间进入本机tmex, 并在后台异步登录其他节点
> 6. 已登录后, 节点应该出现在管理设备中(这里也应该节点状态), 但是那里依旧只有本地设备

> 我试图通过本机tmex访问其他机器时,出现了unexpected application error; 每次都默认重新登录节点的设计并不好,有时候节点数量很多,很多本次都用不上,你应该只在用户需要时连接;
